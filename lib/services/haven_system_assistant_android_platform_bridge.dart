...
typedef HavenSystemAssistantAndroidRequestHandler =
    Future<bool> Function(Map<String, Object?> request);

abstract interface class HavenSystemAssistantAndroidPlatformBackend {
  void setRequestHandler(HavenSystemAssistantAndroidRequestHandler? handler);

  Future<bool> requestPendingDelivery();
}

/// Strict transport for the private Android system-assistant ingress seam.
///
...
final class MethodChannelHavenSystemAssistantAndroidBackend
    implements HavenSystemAssistantAndroidPlatformBackend {
  MethodChannelHavenSystemAssistantAndroidBackend({MethodChannel? channel})
    : _channel = channel ?? const MethodChannel(channelName);

  static const channelName = 'com.focushaven/system_assistant_android';
  ...
  HavenSystemAssistantAndroidRequestHandler? _handler;
  ...
  Future<bool> requestPendingDelivery() async {
    ...
    if (result is! bool) {
      throw const FormatException(
        'Malformed Android system-assistant delivery result.',
      );
    }
    return result;
  }
  ...
  static Map<String, Object?> _readMap(Object? value) {
    if (value is! Map) {
      throw const FormatException(
        'Malformed Android system-assistant request.',
      );
    }
    ...
    for (final entry in value.entries) {
      ...
      if (key is! String) {
        throw const FormatException(
          'Malformed Android system-assistant request key.',
        );
      }
...
/// Owns one memory-only Android-to-Flutter request-delivery session.
///
/// The controller validates the exact three-field wire payload before handing
/// it to [HavenSystemIntentInbox]. Returning `true` acknowledges only that the
/// app-level inbox accepted the request; it never prepares, confirms, or
/// executes an action. Native code retains an unacknowledged request for a
/// later foreground delivery attempt within the same process lifetime.
final class HavenSystemAssistantAndroidPlatformController {
  HavenSystemAssistantAndroidPlatformController({
    required HavenSystemAssistantAndroidPlatformBackend backend,
    required HavenSystemIntentInbox inbox,
  }) : this._(backend, inbox);

  HavenSystemAssistantAndroidPlatformController._(this._backend, this._inbox);
  ...
  final HavenSystemAssistantAndroidPlatformBackend _backend;
  ...
  static HavenSystemIntentRequest _parseRequest(Map<String, Object?> value) {
    ...
    if (value.length != keys.length ||
      ...
      throw const FormatException(
        'Unexpected Android system-assistant request fields.',
      );
    }
    ...
    if (invocationId is! String ||
      ...
      throw const FormatException(
        'Malformed Android system-assistant invocation ID.',
      );
    }
    final kind = switch (value['kind']) {
      ...
      _ => throw const FormatException(
        'Unsupported Android system-assistant route.',
      ),
    };
