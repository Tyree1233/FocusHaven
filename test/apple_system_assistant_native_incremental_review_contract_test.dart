...
void main() {
  ...
  IncrementalLocaleReviewManifest manifest() =>
      IncrementalLocaleReviewManifest.fromJson({
        ...
        'deltaId': 'apple-system-assistant-native-copy-v1',
  ...
  test('locks fifteen independent reviews and Portuguese derivation', () {
    ...
    expect(value.deltaId, 'apple-system-assistant-native-copy-v1');
    expect(
  ...
  test(
    'documentation forbids Flutter runtime and public native integration',
    () {
      ...
      final review = normalize(
        File(
          'docs/APPLE_SYSTEM_ASSISTANT_NATIVE_COPY_REVIEW.md',
        ).readAsStringSync(),
      );
      ...
      expect(review, contains('Apple string catalog only by a separate'));
      expect(review, contains('Provider-assisted drafts require separate'));
      expect(workflow, contains('Apple-native system-assistant copy'));
      expect(workflow, contains('not Flutter runtime content'));
      expect(workflow, contains('public App Intent registration stays closed'));
    },
  );
}
