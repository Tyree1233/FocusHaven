...
Status: living product contract; reviewed Spanish and French are active in the
app while store and country promotion remain separate
...
| Global localization | English, Spanish, French, German, and Brazilian Portuguese active | The production runtime supports five exact reviewed languages, follows supported device language by default, and offers local Device, English, Español, Français, Deutsch, and Português (Brasil) choices. German and Brazilian Portuguese are the first bounded two-locale batch: each retains an independent candidate, fluent review, anonymous validation record, exact runtime lock, and pass/fail result. Flutter's required base `pt` fallback is mechanically identical to the reviewed `pt-BR` messages and is not a separate language choice. The picker remains registry-driven and English remains the fallback. | Reuse the bounded batch path for later languages; add physical-device, speech, font, right-to-left, or store work only when materially required. |
