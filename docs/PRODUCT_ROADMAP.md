...
Status: living product contract; sixteen reviewed languages are active in the app
while store and country promotion remain separate
...
The September 19 cleanup detaches the legacy launcher registration while
preserving its resources, resolver and historical contracts. Current README and
roadmap claims do not promise provider availability. Do not pursue an AppFunctions
or Google Home migration now. Preserve in-app voice, confirmation and validation.
Phase 218 soundscapes are implemented as a default-off preview; signed-release
acceptance remains separate. Historical Phase 217L/217M provider-validation plans below
...
| Siri, Shortcuts, and Android App Actions | Availability-gated Apple registration; external Android invocation deferred | Apple registration and the shared text-free, expiring in-app review boundary remain unchanged. Android retains the reviewed native copy, resolver, replay protection and historical Phase 217K/217L contracts, but the launcher no longer registers the legacy App Actions resource. | Preserve in-app voice and validation; do not promise external Google Assistant/Gemini invocation or pursue provider migration without a separately approved eligible path. Final retained-feature and signed-release gates remain separate. |
| Soundscapes and focus environments | Implemented, default-off preview | Phase 218 provides bundled offline soft noise, explicit sound-only playback/volume controls, local artwork, background media controls and localized metadata. Source/platform checks and owner-reported Spanish/English Moto/iPhone preview checks passed; the translation delta uses AI editorial review with an explicit human-review waiver. No generated environment is implemented. | Keep default-off until production acceptance; validate intended signed release artifacts and disclosures without repeating unchanged preview checks. |
...
between the real controls. The exact corrected signed Spanish app then passed
the short physical VoiceOver retest, including independent Restore and Coach
focus targets and a usable Coach round trip. The exact normal English app was
restored in place and passed the same physical clearance check. Android
TalkBack and iOS VoiceOver acceptance are now complete, so the Spanish
...
of Haven Rhythm, and a qualified Focus Forecast window. It returns one
ephemeral `AdaptiveFocusSuggestion` made only of bounded numbers, booleans,
enums, and an optional timing window.
...
The Phase 216B settlement gate issues one in-memory, single-use ticket for one
exact suggestion. It consumes the ticket before checking the latest suggestion
and current owner values, so stale, replayed, superseded, mismatched, unchanged-
accept, and owner-changed attempts fail closed. Keep-current grants no
...
exactly match the preview. Settlement consumes that capability before
rechecking the latest suggestion and live owner state. Keep-current changes
nothing. A valid acceptance delegates both values to one timer-owned atomic
method, which rechecks expected defaults, persists through existing storage,
and leaves the session stopped. Stale, replayed, superseded, active, paused,
completed, non-Focus, invalid, mismatched, or unchanged requests fail closed.

The preview, tickets, decision, and delegation result are not persisted. Only
the authoritative timer persists a successfully reviewed pair; it cannot start
...
accepts only the seventeen-message delta through the established structural,
placeholder, source-equal, script-contamination, repetition, leaked-data, and
brand-safety gates. One private review remains required for each of the fifteen
target languages, and `pt` remains a mechanical derivative of approved
...
only schema version, a bounded opaque invocation ID, and one allowlisted enum.
It cannot carry a transcript, utterance, task title, coaching history, journal,
reflection, account value, or arbitrary parameter.

The local preparation service consumes each valid invocation ID once within
one bounded ephemeral service lifetime and maps the request only into the
...
Phase 217C adds the isolated `HavenSystemIntentReviewCard` presentation
foundation. It receives only one opaque `HavenSystemIntentReview`, one complete
`HavenSystemIntentReviewCopy`, and separate dismiss and confirm callbacks. It
can display the already-localized interpretation and effect exposed by the
review, but it cannot access the private proposal or call the review service or
engine. The complete copy contract keeps the source disclosure, local privacy
boundary, two-minute freshness warning, exact-confirmation disclosure, risk
label, semantic summary, and both action labels out of fragment-based runtime
assembly.

The complete summary is one live semantic region, while dismiss and confirm
remain separate semantic buttons. Selecting either disables both controls
before returning the exact opaque review to that one callback. A different
review identity resets the presentation, but replay and settlement authority
remain exclusively with the Phase 217B service and engine. The widget imports
no localization, timer, queue, provider, persistence, network, AI, or native
platform owner.

Phase 217C adds no runtime catalog or source-specific copy, production
provider or screen consumer, deep link, persistence, dependency, permission,
native manifest entry, Siri/App Intent, Shortcut, or Android App Action. The
complete source-specific copy must receive independent language review before
a later production host can consume the card, and native registration remains
a subsequent release gate.

Phase 217D English production-copy proposal locked eleven complete messages for
that presentation in an isolated ARB proposal outside Flutter runtime localization. The proposal covers
the source and no-action disclosure, exact text-free privacy boundary,
two-minute freshness and stale-state warning, mandatory in-app confirmation,
separate informational and reversible-control risk explanations, one complete
semantic summary, and the two explicit action labels.

The summary accepts only the already-localized interpretation, effect, and
risk values. Every language may reorder those placeholders inside one complete
reviewed sentence; no production layer may concatenate translated fragments.
The proposal contains no private user or reviewer data and grants no
system-assistant or Haven Action authority.

The bounded incremental-review foundation locks the proposal and all current
runtime-catalog digests and reuses the established review engine for only this
eleven-message delta across the fifteen non-English production languages.
Japanese and Korean retain explicit font-coverage gates. Each language still
requires independent fluent approval, and `pt` may be derived only from a
completely approved `pt-BR` delta. The foundation itself creates no provider
draft, real workbook, approval, runtime merge, generated localization, or
production consumer.

The subsequent Phase 217D integration accepts all 165 fluent decisions,
derives `pt` only from approved `pt-BR`, merges the eleven complete messages
into all seventeen runtime catalogs, and adds one app-level host. The host
receives only text-free Phase 217A requests through a bounded, memory-only
inbox, creates one opaque review through the Phase 217B service, and removes it
on dismissal, confirmation, supersession, expiry, or owner-state change.
Settlement still requires the exact in-app Confirm action and existing Haven
...
Flutter runtime copy. The original copy-lock foundation created no provider
draft, real workbook, approval, Apple string catalog, native accessor, App
Intent, Shortcut provider, entitlement, production registration, or execution
path.

All fifteen approvals and the exact native string-catalog transform are now
complete. The catalog contains English, fifteen independently reviewed
localizations, and base Portuguese derived only from approved Brazilian
Portuguese; its typed accessor owns no request or execution behavior.
...
Phase 217G availability-gated Apple registration adds exactly five
parameter-free `AppIntent` declarations and five matching `AppShortcut`
entries on iOS 16 and later. The shortcut phrases are mechanically derived
from the reviewed Phase 217F phrase values into a localized
`AppShortcuts.strings` family, with
exactly one `${applicationName}` token in each of seventeen locales. Runner's
iOS 15 deployment target remains unchanged, and registration setup is guarded
by an iOS 16 availability check.

Every intent requires authentication and opens FocusHaven for the existing
in-app review. Its only native mutation is an attempted write of schema version
`1`, a generated bounded opaque invocation ID, and one allowlisted route to the
Phase 217E process-memory slot. An occupied slot, malformed identifier, or
missing reviewed copy fails closed. The native result can report only ready for
review, pending review, or unavailable; it never reports timer or queue success
and never confirms or executes an action.

Phase 217G adds no parameter, transcript, utterance, duration, task, queue-item,
deep link, persistence, third-party dependency, Siri entitlement, Android
manifest entry, timer or queue owner, review settlement, or Haven Action Engine
call. Real-device cold- and warm-launch Siri/Shortcuts behavior, VoiceOver and
large-text review in supported languages, signed release builds, privacy and
store-disclosure review, candidate validation, and distribution authorization
remain closed release gates. Android App Actions remain a separate copy,
adapter, review, and release phase.

Phase 217H adds one Android-only native ingress seam without publishing an App
Action or shortcut. Kotlin and Dart independently admit only schema version
`1`, a bounded opaque invocation ID, and the same five-route enum. Unknown
fields, malformed identifiers, unsupported routes, free-form parameters, and a
second pending request fail closed. The one native slot exists only in process
memory and clears only after the existing Flutter inbox acknowledges the exact
request.

`MainActivity` installs the transport, and an Android-only lifecycle host asks
for delivery after Flutter is ready and whenever the app resumes. Delivery
terminates at the Phase 217D memory inbox. It does not prepare, confirm, or
settle a review and imports no timer, queue, review-service, or Haven Action
owner.

Phase 217H adds no `shortcuts.xml`, `android.app.shortcuts` manifest metadata,
built-in or custom Assistant intent, query pattern, public Android copy, deep
link, exported activity, permission, dependency, persistence, provider,
network, AI, or execution path. Android discovery copy, independent
fifteen-language review, exact capability mapping, official Assistant preview,
real-device TalkBack and large-text testing, signed builds, Play disclosure
review, candidate validation, and explicit distribution authorization remain
closed gates.

Phase 217I locks the complete English source for the Android-native discovery
surface as twenty-eight messages with matching metadata: collection and
discovery copy, privacy and no-parameter summaries, one review instruction,
five short labels, five long labels, five review-only invocation examples, six
truthful outcomes, and two TalkBack labels. The proposal contains no
placeholder. Every route prepares a request for review; no string claims that
submission started, paused, resumed, read, or opened anything.

Android `shortcuts.xml` may later reference localized short and long labels,
but built-in intents model supported queries independently and custom intents
use separate query-pattern resources with a narrower locale contract. Phase
217I therefore chooses neither a built-in nor custom intent and creates no
query pattern, capability, fulfillment, static shortcut, or manifest metadata.

The external incremental-review workflow accepted fifteen independent reviews
with explicit Japanese and Korean font-coverage gates. The exact approved
values now populate seventeen localized Android resource files. English is the
base, reviewed Brazilian Portuguese maps to `values-pt-rBR`, base Portuguese is
its byte-identical derivative, and reviewed Indonesian maps from `id`
provenance to Android's `values-in` qualifier. A typed Kotlin accessor maps only
the same twenty-eight resource keys and fails closed without submitting a
request. Flutter runtime-catalog hashes remain freshness locks because this is
Android-native, not Flutter runtime copy.

The reviewed Android-native resource catalog creates no `shortcuts.xml`,
capability, query pattern, manifest change, fulfillment mapping, public
registration, or execution path. Exact capability mapping, official Assistant
preview, real-device TalkBack and supported-
...
### Phase 217K — Android App Actions registration foundation

Implement the exact reviewed Phase 217J mapping with one `shortcuts.xml`, four
en-US custom-intent query arrays, one constant `OPEN_APP_FEATURE` inventory,
launcher-activity metadata, the required AndroidX Core dependency, and one
fail-closed resolver. Timer routes accept no extras. Queue review accepts
exactly one `feature` extra equal to `focus_queue_review`; the value is
validated and discarded before the existing schema-version, opaque-ID, and
route-kind request is submitted.

Cold and warm fulfillment share the same resolver. Unknown actions, changed or
additional extras, URI data, `ClipData`, selectors, missing reviewed copy, and
malformed generated IDs fail closed. After a recognized attempt, the activity
retains only a neutral package-scoped launch intent, preventing recreation
from replaying the public fulfillment. Submission remains separate from the
visible **Confirm action** and never reads or changes timer or queue state.
...
Implemented as a default-off preview and merged in `f5899fc`: local, user-selected
soft noise with predictable offline playback, explicit sound-only controls,
background media metadata and local branding. The sixteen-message delta covers
fifteen target languages plus English and the derived Portuguese fallback; its
review is AI editorial with an explicit owner waiver, not fluent-human approval.
Flutter tests/analysis, Android checks, unsigned iOS compilation, and scoped
Spanish/English TalkBack/VoiceOver and media checks passed. PR and main-branch
CI passed. These preview results are not signed production-release validation.
See [the detailed evidence and remaining gates](OFFLINE_SOUNDSCAPES_IMPLEMENTATION.md).

Generated environments remain future work: opt-in, cost-bounded, independently
moderated, and never required for core focus.
