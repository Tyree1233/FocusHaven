...
| Global localization | Spanish active in app | The production runtime supports the exact reviewed English and Spanish catalogs, follows supported device language by default, and offers local Device, English, and Español choices. English is the fallback; French, German, and Brazilian Portuguese remain planned. Store promotion remains separate, Spanish commands fail closed when unsupported, and Enhanced AI remains disabled. | Reuse the streamlined translate, private-review, validate, activate, test, and CI path for later locales; add exceptional device or store work only when materially required. |
