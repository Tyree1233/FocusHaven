...
| Global localization | English, Spanish, and French active | The production runtime supports the exact reviewed English, Spanish, and French catalogs, follows supported device language by default, and offers local Device, English, Español, and Français choices. French is the first locale delivered through the compact generic candidate, private review, anonymous acceptance, exact-lock integration, shared-test, and single-CI path. The picker remains registry-driven. English is the fallback; German and Brazilian Portuguese remain planned. | Run German through the same streamlined path; add exceptional device or store work only when materially required. |
