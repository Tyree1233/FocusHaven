...
| Global localization | Spanish active; streamlined next-locale pipeline implemented | The production runtime supports the exact reviewed English and Spanish catalogs, follows supported device language by default, and offers local Device, English, and Español choices. A generic candidate, private CSV review, anonymous acceptance, and exact-lock verification tool now replaces the Spanish foundation's repeated preparation phases. The production language picker is registry-driven. English is the fallback; French, German, and Brazilian Portuguese remain planned. | Run French through the compact prepare, private-review, accept, integrate, activate, test, and single-CI path; add exceptional device or store work only when materially required. |
