...
Status: living product contract; seven reviewed languages are active in the app
...
| Global localization | Seven reviewed in-app languages active | The production runtime supports English, Spanish, French, German, Brazilian Portuguese, Japanese, and Korean; follows a supported device language by default; and provides matching local Appearance choices. German and Brazilian Portuguese retain independent batch reviews and runtime locks. Japanese and Korean additionally passed exact physical Android and iOS CJK glyph, fallback, wrapping, large-text, branding, repetition, contamination, and control-clearance checks before activation. Flutter's required base `pt` fallback remains a mechanical derivative rather than a separate language choice. The picker remains registry-driven and English remains the fallback. | Complete the independent reviews for Italian, Polish, and Dutch; keep speech, right-to-left, store promotion, and country distribution behind their separate gates. |
