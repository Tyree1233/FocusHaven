...
This revision records the Phase 212 Voice-to-Coach and Phase 213 Safe Voice
Commands source boundary. It does not claim that the microphone permission,
speech-recognition purpose, speech dependency, or voice-action flow is covered
by any earlier signed or store-validated artifact. Final answers require a new
...
That candidate predates Phases 212 and 213 and contains neither Voice-to-Coach,
...
A future Phase 212/213 archive audit must separately resolve Apple **Audio
Data** for the exact speech-recognition path. Apple documents that
...
| Audio files — Voice or sound recordings | Requires final provider mapping | Coach and Haven-action voice input; explicit and ephemeral | Speech transcription. If the selected recognizer transmits audio off device, Google requires it in the Data safety response even when processing is ephemeral; the final form must match the exact runtime provider behavior. |
...
5. Voice-to-Coach and Safe Voice Commands begin only after an informed tap,
   have a visible bounded listening state, keep no raw audio, and leave the
   transcript editable. Coach text remains unsent until Send. Action text
   remains unproposed until Review action and cannot run until the person uses
   the separate visual Run reviewed action or Confirm exact action control.
   Denial leaves typing and normal controls usable.
